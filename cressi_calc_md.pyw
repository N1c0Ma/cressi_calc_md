...
def parse_TG(in_file):
    global int_d, rat_d, depths_num
    for files in in_file:
        ...
        depths = str(lines[3:4])    #choose lines for depths-data
        depths = depths.split("\\t") #split delimiter "\t"
        depths = depths[1:]   #choose data-points
        print(depths)
        for i in range(len(depths)):
            int_d10.append(depths[i][0])  #make integer ten num-list
            if depths[i][1].isdecimal():
                int_d1.append(depths[i][1])  #append if number of ones = True as string
            else:
            ...
            if depths[i][2].isdecimal(): 
                rat_d.append(depths[i][2])  #make rational num-list if [2] is decimal
            else:
                rat_d.append(depths[i][3])   #if not take [3]
            try:
                int_d[i] = int(int_d[i])    #convert to int
                rat_d[i] = int(rat_d[i])    #convert to int
            except notallnumbers:
                output_sol.insert(END, "error")
        depths_num = len(depths)    #number of depths datapoints
        calc_md(int_d, rat_d, depths_num)
                    
def calc_md(int_d, rat_d, depths_num):
    md = round((sum(int_d) + sum(rat_d) / 10) / depths_num, 1)  #make sums, divide rational sum / 10, divide through num data-points, round
